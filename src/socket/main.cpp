...
void* send_handle(void* client_socket_p){
...
int main(){
    ...
    assert(0==bind(server_socket, (struct sockaddr *)&server_addr, sizeof(server_addr)));   //将本地地址绑定到所创建的套接字上
    ...
    {   
        ...
        pthread_t tid;
        int error;
        error=pthread_create(&tid,NULL,&send_handle,&client_socket);
